...
contract MockRubiconMarket is MatchingEvents, ExpiringMarket, DSNote {
    ...
    /// @dev Create offer, avoid crossing the spread.
    function offerPostOnly(
        uint256 pay_amt,
        IERC20 pay_gem,
        uint256 buy_amt,
        IERC20 buy_gem,
        ...
        /// @dev Best offer, from the other side of the book.
        (uint256 m_pay_amt, , uint256 m_buy_amt, ) = getOffer(
            getBestOffer(buy_gem, pay_gem)
        );

        /// @dev Check if new offer won't cross the spread.
        if (
            mul(m_buy_amt, buy_amt) <
            mul(pay_amt, m_pay_amt) +
                (matching ? m_buy_amt + buy_amt + pay_amt + m_pay_amt : 0)
        ) {
            /// @dev Implicitly fail, without revert.
            return 0;
        }

        /// @dev Safely create 'post only' order.
        return
            offer(
                pay_amt,
                pay_gem,
    ...
    // *** Batch Functionality ***
    /// @dev Arguments to the simplest offer entry-point.
    struct BatchOfferStep {
        uint256 payAmt;
        address payGem;
        uint256 buyAmt;
        address buyGem;
    ...
    /// @dev Arguments to the extended offer entry-point.
    struct OptionalOfferInfo {
        uint256 pos;
        bool rounding;
        address owner;
        address recipient;
    ...
    /// @notice Batch offer functionality - multuple offers in a single transaction
    function batchOffer(
        BatchOfferStep[] calldata steps,
        OptionalOfferInfo[] calldata info
    ) public {
        require(steps.length == info.length, "Array lengths do not match");

        for (uint256 i = 0; i < steps.length; i++) {
            offer(
                steps[i].payAmt,
                IERC20(steps[i].payGem),
                steps[i].buyAmt,
                IERC20(steps[i].buyGem),
                info[i].pos,
                info[i].rounding,
                info[i].owner,
                info[i].recipient
    ...
    /// @dev Cancels only offers, that are active, skipping the others.
    function batchOfferPostOnly(
        BatchOfferStep[] calldata steps,
        OptionalOfferInfo[] calldata info
    ) external {
        require(steps.length == info.length, "Array lengths do not match");
    ...
    /// @return canceled - number of succesfully canceled orders
    function batchCancel(
        uint[] calldata ids
    ) public returns (uint256 canceled) {
    ...
    function batchRequote(
        ...
        batchCancel(ids);
        batchOffer(payAmts, payGems, buyAmts, buyGems);
    }
